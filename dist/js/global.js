...
(function ($) {
    ...
    $.fn.SumoSelect = function (options) {
        ...
        var settings = $.extend({
            ...
            floatWidth: 639,              // Screen width of device at which the list is rendered in floating popup fashion.
...
(function ($, window, document) {
  ...
  var page = {
    ...
    searchPanel: function () {
      ...
      var searchTrigger = searchPanel.find('.btn-trigger');
      searchTrigger.on('click', function () {
        if (!searchPanel.hasClass('focused')) {
          searchPanel.addClass('focused');
        }
      });
      $(document).on('click', function (e) {
        if ($(e.target).closest(searchPanel).length === 0) {
          searchPanel.removeClass('focused');
        }
      });
    },
    customForm: function () {
    ...
    resultTabset: function () {
      $('.tabs').find('li').on('click', function () {
        var $this = $(this);
        $this.addClass('current').siblings().removeClass('current');
        $('.tab-content').hide().removeClass('current');
        $("#" + $this.attr('data-tab')).fadeIn().addClass('current');
      });
    },
    popup: function () {
      $('.popup-link').fancybox({
    ...
    uploadLink: function () {
      $(document).on('click', '#upload_link', function (e) {
        ...
        uploadFile.change(function () {
          ...
          if (file) {
            $('#upload_link').text(file.name);
          }
        });
      });
    },
    ajax_create_event: function () {
      $('#form_create_event').submit(function (event) {
        ...
        $.ajax({
          ...
          url: url_create, // Url to which the request is send
          ...
          {
            if (response == 'not-load') {
              $('.info-load').addClass('error-info').text('Please upload a photo.');
            } else if (response == 'not-size') {
              $('.info-load').addClass('error-info').text('The size of uploaded images not more than 1MB');
            } else if (response == 'not-type') {
              $('.info-load').addClass('error-info').text('Does not match the extension of the uploaded file');
            } else if (response == 'ok') {
              $('#form_create_event').trigger('reset');
              $('#upload_link').text('Upload photo');
              $('.info-load').removeClass('error-info').text('The size of uploaded images not more than 1MB');
              $('#create-event').find('.popup-close').trigger('click');
              var eventForm = document.getElementById('form_create_event');
              var formSelects = eventForm.getElementsByTagName('select');
              for (var i = 0; i < formSelects.length; i++) {
                formSelects[i].sumo.unload();
                $(formSelects[i]).children('option').first().prop('selected', true);
                $('select').SumoSelect();
              }
              $.fancybox({
                'href': '#thanks',
